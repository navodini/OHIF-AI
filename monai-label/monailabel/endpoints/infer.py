...
def run_inference(
    ...
    if output == "dicom_seg":
        ...
        # Handle known error/status file paths that should return as JSON response
        known_status_paths = [
            "/code/predictions/reset.nii.gz",
            "/code/predictions/init.nii.gz",
            "/code/predictions/sam3_not_found.nii.gz",
            "/code/predictions/nnunet_not_available.nii.gz",
            "/code/predictions/nnunet_model_not_found.nii.gz",
            "/code/predictions/nnunet_error.nii.gz",
        ]
        if type(res_img) == str and res_img in known_status_paths:
